...
class ArUcoDetector:
    def __init__(self, settings: ArUcoSettings, channel: Channel, subscription: Subscription):
    ...
    def localize(self,
        ...
        intrinsic = np.dot(scale, self.to_np(tensor=calibration.intrinsic))
        distorsion = self.to_np(tensor=calibration.distortion)
        ...
        for index in range(len(annotations.objects)):
            ...
            transformation.tf.CopyFrom(self.to_tensor(array=mat))
    ...
    def run(self,
        ...
        message = Message(content=annotations)
        message.topic = "ArUco.{}.Detection".format(camera_id)
